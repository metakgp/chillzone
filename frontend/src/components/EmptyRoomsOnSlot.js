...
class EmptyRooms extends Component {
  ...
  render() {
    ...
    if (!(day >= 0 && day < 5 && slot >= 0 && slot < 9)) {
      return (
        <h3>
          That day/slot ({day}/{slot}) combination is invalid. 0 &lt;= Day &lt;
          5 and 0 &lt;= Slot &lt; 9
        </h3>
      );
    }
    ...
    if (Complexes[complex] === "V") {
      ...
      schedule = schedule.filter((item) => {
        return (
          item.includes(Complexes[complex] + Floors[floor]) &&
          !item.includes("V")
        );
      });
    }
    ...
    if (this.props.show_common_next) {
      ...
      common_rooms = intersection(
        this.props.schedule[day][slot],
        this.props.schedule[next_day][next_slot]
      );
    }
    ...
    if (this.props.show_common_prev) {
      ...
      common_rooms = intersection(
        this.props.schedule[day][slot],
        this.props.schedule[prev_day][prev_slot]
      );
    }
